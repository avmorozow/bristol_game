import {tapBoundary} from './tap-plan';
 ...
 */
export type TapPosition = {id:string;status:string;tap:number;outcomes?:boolean[]};
type Options = {
...
export class TapQueue {
 ...
 add():boolean {
  ...
  if(this.preview().tap>=tapBoundary(a))return false;
 ...
 preview(){
  ...
  const tap=Math.min(tapBoundary(a),a.tap+this.queued+unresolved);
