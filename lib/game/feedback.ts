...
export function encouragement(a:Attempt|null|undefined){
 if(!a)return '120 тапов. Сколько заберёшь ты?';
 ...
 if(a.status==='loss_pending')return 'Пакет у белки. Вернёшь своё?';
 ...
 if(a.boosterUsed)return 'Одну отогнали. Вторая — без права на спасение.';
 ...
 if(a.tap>0)return 'Ещё один тап — или забрать своё?';
 return 'Соберёшь все 120?';
}
