...
type Modal='rules'|'history'|'referral'|'paid'|'tutorial'|'exit'|'demo'|'gifts'|'settings'|null;
const A='/assets/';
...
export default function Game(){
 ...
 const send=useCallback(async(action:string,value?:unknown):Promise<State|null>=>{
  ...
  try{
   ...
   if((cmd.action==='tap'||cmd.action==='taps')&&data.attempt&&data.attempt.status!==before?.attempt?.status){
 ...
 async function start(kind=selectedMode){const ready=current.current??await refresh();if(!ready)return;if(!ready.tutorial){setStep(0);setModal('tutorial');return;}if(kind==='paid'){setModal('paid');return;}if(!ready.freeAvailable){toast('Бесплатная попытка обновится в 03:00 по Москве');return;}const s=await send('start','free');if(s)setModal(null);}
 ...
 function tap(){
  ...
  const before=tapPreview(position,queue.preview().tap);if(!queue.add())return;
  const after=tapPreview(position,queue.preview().tap),id=++particleId.current;
  setParticles(p=>[...p.slice(-11),{id,amount:after.reward-before.reward,left:24+(id*19)%52}]);
 ...
 return <main className="game-shell" onPointerDownCapture={()=>audioEngine.current?.unlock()} onKeyDownCapture={()=>audioEngine.current?.unlock()}>
  ...
  <section className={`game-stage ${onHome?'home-scene':'play-scene'}`} aria-label="Игра Собери пакет">
   ...
   {onHome&&<div className="home-content">
    ...
    <div className="home-bottom mode-home">
     <fieldset className="mode-picker"><legend>Выбери свою попытку</legend>
      <label className={`mode-card ${selectedMode==='free'?'selected':''} ${!state?.freeAvailable?'unavailable':''}`}><input type="radio" name="attempt-mode" value="free" checked={selectedMode==='free'} disabled={!state?.freeAvailable||blocked} onChange={()=>setSelectedMode('free')}/><span><strong>Бесплатная</strong><small>{state?.freeAvailable?'1 раз в день':state?`Через ${resetTime}`:'Загрузка…'}</small><b>До 2 350 <Coin size={18}/></b></span></label>
      <label className={`mode-card ${selectedMode==='paid'?'selected':''}`}><input type="radio" name="attempt-mode" value="paid" checked={selectedMode==='paid'} disabled={blocked} onChange={()=>setSelectedMode('paid')}/><span><strong>За 100 монет</strong><small>В любое время</small><b>До 5 000 <Coin size={18}/></b></span></label>
     </fieldset>
     <Action onClick={()=>void start()} disabled={blocked||(selectedMode==='free'&&!state?.freeAvailable)} className="play-button">{loading?<Loader2 className="spin"/>:selectedMode==='free'?'ИГРАТЬ БЕСПЛАТНО':<>ИГРАТЬ ЗА 100 <Coin size={27}/></>}</Action>
    </div>
   </div>}
   ...
   {(playing||result)&&<>
    <div className="score-zone">
     ...
     {state?.nextTap&&<p className="risk-info"><span>{preview.pending?<>Подтверждено: <strong>{coinCount(a?.reward??0)}</strong> · уточняем сумму</>:<>Следующий удачный тап: <strong>{coinCount(state.nextTap.reward)}</strong></>}</span><span>{preview.pending?'Ближайший риск белки: ':'Шанс белки: '}<strong>{state.nextTap.lossPercent.toLocaleString('ru-RU',{maximumFractionDigits:1})}%</strong></span></p>}
    </div>
  ...
  <Dialog open={a?.status==='loss_pending'&&!modal} onOpenChange={()=>{}}>
   <DialogContent ref={lossFocus} tabIndex={-1} onOpenAutoFocus={e=>{e.preventDefault();lossFocus.current?.focus();}} className="game-modal illustration-modal" showCloseButton={false} onEscapeKeyDown={e=>e.preventDefault()} onPointerDownOutside={e=>e.preventDefault()}>
    <img className="modal-art thief-art" src={A+'thief.png'} alt="Белка поймала пакет"/>
    <div className="modal-body"><DialogTitle>Отогнать белку?</DialogTitle><DialogDescription>Верни выигрыш за 50 монет. После спасения можно сразу забрать его или продолжить. Следующая белка завершит попытку.</DialogDescription><div className="at-risk"><span>На кону</span><strong>{fmt(a?.reward??0)} <Coin/></strong></div>
    {(a?.reward??0)<=50&&<p className="decision-note">Спасение стоит 50 монет, а на кону — {coinCount(a?.reward??0)}. Продолжение остаётся рискованным.</p>}
    ...
    <Action secondary onClick={()=>void send('lose')} disabled={blocked}>ЗАКОНЧИТЬ ИГРУ</Action>{error&&<p className="field-error">{error}</p>}{pending&&!busy&&<Action secondary onClick={()=>void send('retry')} disabled={busy}>Проверить операцию</Action>}</div>
   </DialogContent>
  </Dialog>
