...
export default function Game(){
 ...
 const apply=useCallback((s:State)=>{if(!current.current||s.referralCode!==current.current.referralCode||s.revision>=current.current.revision){current.current=s;setState(s);}},[]);
 ...
 async function start(){const ready=current.current??await refresh();if(!ready)return;if(!ready.tutorial){setStep(0);setModal('tutorial');return;}if(!ready.freeAvailable){setModal('paid');return;}await send('start');}
 ...
 return <main className="game-shell">
  ...
  <section className={`game-stage ${onHome?'home-scene':'play-scene'}`} aria-label="Игра Собери пакет">
   ...
   {(playing||result)&&<>
    <div className="score-zone">
     <p className="score-caption">{showForced?'Показательный сценарий':a?.status==='final_ready'?'Все 120 тапов пройдены!':a?.tap?'Можно забрать сейчас':'Нажми на пакет'}</p>
     ...
     {state?.nextTap&&<p className="risk-info"><span>Следующий удачный тап: <strong>{coinCount(state.nextTap.reward)}</strong></span><span>Шанс белки: <strong>{state.nextTap.lossPercent.toLocaleString('ru-RU',{maximumFractionDigits:1})}%</strong></span></p>}
    </div>
    <div className="tap-area">
     ...
     {a?.status==='active'&&<p className="tap-hint">{tapBacklog?`Принято нажатий: ${tapBacklog} · проверяем результат`:a.tap===0?'Нажимай на пакет':a.boosterUsed?'Следующая белка завершит игру':'Продолжай или забери монеты'}</p>}
    </div>
  ...
  <Dialog open={!!a&&['final_ready','won','lost','abandoned'].includes(a.status)&&!modal} onOpenChange={open=>{if(!open&&a?.status!=='final_ready')dismissResult();}}>
   <DialogContent ref={resultFocus} tabIndex={-1} onOpenAutoFocus={e=>{e.preventDefault();resultFocus.current?.focus();}} className="game-modal illustration-modal" showCloseButton={false} onEscapeKeyDown={e=>{if(a?.status==='final_ready')e.preventDefault();}} onPointerDownOutside={e=>{if(a?.status==='final_ready')e.preventDefault();}}>
    ...
    {a?.status==='final_ready'?<Action onClick={cashout} disabled={blocked}>ЗАБРАТЬ НАГРАДУ</Action>:<><Action onClick={()=>void start()} disabled={blocked}>{state?.freeAvailable?'ИГРАТЬ БЕСПЛАТНО':<>ЗАНОВО ЗА 100 <Coin/></>}</Action>{a?.coupon&&<Action secondary onClick={()=>setModal('gifts')}>МОЙ ПОДАРОК</Action>}<button className="text-button" onClick={()=>void dismissResult()} disabled={blocked}>На главную</button></>}
