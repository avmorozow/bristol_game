...
import {GameCharacter,CharacterReaction} from '@/components/game-character';
import type {CharacterMoment} from '@/components/game-character';
...
export default function Game(){
 ...
 const [moment,setMoment]=useState<CharacterMoment|null>(null),[motionHidden,setMotionHidden]=useState(false);
 useEffect(()=>{if(!moment)return;const timer=setTimeout(()=>setMoment(null),1100);return()=>clearTimeout(timer);},[moment]);
 ...
 const apply=useCallback((s:State)=>{if(!current.current||s.referralCode!==current.current.referralCode||s.revision>=current.current.revision){if(s.attempt?.id!==current.current?.attempt?.id){setProjectedTap(s.attempt?.tap??0);setTapBacklog(0);setParticles([]);}const previous=current.current?.attempt,next=s.attempt;if(previous&&next&&previous.id===next.id){if(!previous.boosterUsed&&next.boosterUsed&&['active','final_ready'].includes(next.status))setMoment({id:next.id+':rescue',kind:'rescue',tap:next.tap});else if(next.status==='active'&&Math.floor(next.tap/30)>Math.floor(previous.tap/30))setMoment({id:next.id+':'+next.tap,kind:'milestone',tap:Math.floor(next.tap/30)*30});}else setMoment(null);const received=newWalletCredit(current.current,s);if(received)setCredit(received);current.current=s;serverOffset.current=s.serverTime-Date.now();setState(s);}},[]);
 ...
 async function start(kind=selectedMode){
  ...
  if(!ready.tutorial){setStep(0);setModal('tutorial');return;}
 ...
 return <main className="game-shell" onPointerDownCapture={()=>audioEngine.current?.unlock()} onKeyDownCapture={()=>audioEngine.current?.unlock()}>
  ...
  <section className={`game-stage ${onHome?'home-scene':'play-scene'}`} aria-label="Игра Собери пакет" data-motion-paused={motionHidden||!!modal||result||a?.status==='loss_pending'}>
   ...
   {onHome&&<div className="home-content">
    <div className="home-banner"><GameCharacter className="home-mascot"/><div><h1>Зарабатывай<br/>монеты</h1><p>Тапай по пакету и забирай<br/>выигрыш вовремя</p></div></div>
   ...
   {(playing||result)&&<>
    ...
    <div className="tap-area" key={a?.id}>
     {moment&&<CharacterReaction key={moment.id} moment={moment}/>}
     <button ref={bagElement} className="bag-button" onPointerDown={e=>{if(e.button===0)tap();}} onClick={e=>{if(e.detail===0)tap();}} disabled={tapBlocked||a?.status!=='active'||preview.tap>=120} aria-label="Нажать на пакет"><GameCharacter alt="Пакет Бристоль — объёмный персонаж" mood={a?.status==='active'?'idle':a?.status==='won'||a?.status==='final_ready'?'happy':'still'}/></button>
  ...
  <Dialog open={a?.status==='loss_pending'&&!modal} onOpenChange={()=>{}}>
   <DialogContent ref={lossFocus} tabIndex={-1} onOpenAutoFocus={e=>{e.preventDefault();lossFocus.current?.focus();}} className="game-modal illustration-modal" showCloseButton={false} onEscapeKeyDown={e=>e.preventDefault()} onPointerDownOutside={e=>e.preventDefault()}>
    <GameCharacter kind="squirrel" mood="caught" className="modal-character" alt="Белка поймала пакет"/>
  ...
  {a&&(result||a.status==='final_ready')&&<Dialog open={!modal} onOpenChange={open=>{if(!open&&a?.status!=='final_ready')dismissResult();}}>
   <DialogContent ref={resultFocus} tabIndex={-1} onOpenAutoFocus={e=>{e.preventDefault();resultFocus.current?.focus();}} className="game-modal illustration-modal" showCloseButton={false} onEscapeKeyDown={e=>{if(a?.status==='final_ready')e.preventDefault();}} onPointerDownOutside={e=>{if(a?.status==='final_ready')e.preventDefault();}}>
    <GameCharacter key={a.status} kind={a.status==='lost'?'squirrel':'bag'} mood={a.status==='lost'?'lost':a.status==='abandoned'?'still':'happy'} className="modal-character result-character"/>
