...
import {CONFIG,type Player} from '@/lib/game/engine';
...
const tutorial=[
 ...
 {title:'Остерегайся белки',text:`Белка может забрать весь выигрыш. Один раз за игру её можно отогнать за ${CONFIG.boosterPrice} монет.`,image:'thief.png'},
...
export default function Game(){
 ...
 const spent=(a?.kind==='paid'?100:0)+(a?.boosterUsed?CONFIG.boosterPrice:0),net=(a?.status==='lost'||a?.status==='abandoned'?0:a?.reward??0)-spent;
 ...
 return <main className="game-shell" onPointerDownCapture={()=>audioEngine.current?.unlock()} onKeyDownCapture={()=>audioEngine.current?.unlock()}>
  ...
  <Dialog open={a?.status==='loss_pending'&&!modal&&!heistActive} onOpenChange={()=>{}}>
   <GameDecisionDialog ref={lossFocus} tabIndex={-1} onOpenAutoFocus={e=>{e.preventDefault();lossFocus.current?.focus();}} className={`game-modal scene-decision squirrel-popup squirrel-path-${squirrelVariant}`} onEscapeKeyDown={e=>e.preventDefault()} onPointerDownOutside={e=>e.preventDefault()}>
    ...
    <Action onClick={()=>void decide('booster')} disabled={decisionWaiting||(pending&&!busy)||(state?.balance??0)<(state?.config.boosterPrice??CONFIG.boosterPrice)}>ОТОГНАТЬ ЗА {state?.config.boosterPrice??CONFIG.boosterPrice} <Coin/></Action>
    {(state?.balance??0)<(state?.config.boosterPrice??CONFIG.boosterPrice)&&<p className="field-error">Не хватает монет для спасения</p>}
