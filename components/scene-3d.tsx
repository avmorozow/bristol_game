...
export const Scene3D=forwardRef<SceneHandle,Props>(function Scene3D(props,ref){
 ...
 useEffect(()=>{
  ...
  void (async()=>{try{
   ...
   const floor=new T.Group();floor.userData.ground=true;floor.visible=!live.current.presentation||live.current.presentation==='game';scene.add(floor);
   ...
   const render=(now:number)=>{if(dead)return;frame=requestAnimationFrame(render);if(now-last<1000/(software?24:45))return;const dt=Math.min(.25,(now-last)/1000||.016);last=now;const p=live.current;
    ...
    const sad=p.presentation==='loss';m.sadMouth.visible=sad;m.sadBrows.visible=sad;m.brows.forEach(b=>b.visible=!sad);
    if(sad){
     // A separate result pose: no chase, fear mouth, victory jump or raised arms.
     m.bag.visible=true;m.bag.position.set(0,reduced.matches?0:Math.sin(time*1.8)*.012,0);m.bag.scale.set(1,.97,1);
     m.bag.rotation.set(.06,-.18,reduced.matches?-.025:-.025-Math.sin(Math.min(t,.7)/.7*Math.PI)*.045);
     m.fear.visible=false;m.smile.visible=false;m.eyes.forEach(e=>e.scale.y=.55);m.bagArms.forEach((arm,i)=>arm.rotation.z=i===0?.15:-.15);el.dataset.bagEmotion='sad';
    }
